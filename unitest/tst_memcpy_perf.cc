...
init_buffers(void)
{
	...
	ret = posix_memalign((void**)&large_buf_read, ALIGNMENT_UNIT, LARGE_BUFFER_SIZE + ALIGNMENT_UNIT);
	if (ret != 0)
		goto error_large_buf_read;

	ret = posix_memalign((void**)&large_buf_write, ALIGNMENT_UNIT, LARGE_BUFFER_SIZE + ALIGNMENT_UNIT);
	if (ret != 0)
		goto error_large_buf_write;

	ret = posix_memalign((void**)&small_buf_read, ALIGNMENT_UNIT, LARGE_BUFFER_SIZE + ALIGNMENT_UNIT);
	if (ret != 0)
		goto error_small_buf_read;

	ret = posix_memalign((void**)&small_buf_write, ALIGNMENT_UNIT, LARGE_BUFFER_SIZE + ALIGNMENT_UNIT);
	if (ret != 0)
